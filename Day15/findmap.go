...
func updateChar(char byte, inc int) byte {
	res := char - 49
	res += byte(inc)
	res %= 9
	res += 49
...
func IncrementWeight(field []string) []string {
	var incmap []string
	for y := 0; y < len(field); y++ {
		line := ""
		for x := 0; x < len(field[y]); x++ {
			line += string(updateChar(field[y][x], 1))
		}
		incmap = append(incmap, line)
	}
	return incmap
}

func ExpandMap(field []string, m int) []string {
	var expanded []string
	var temp []string = field
	for my := 0; my < m+1; my++ {
		/// add incmap to expanded m times
		for _, lt := range temp {
			expanded = append(expanded, lt)
		}

		tx := IncrementWeight(temp)
		for mx := 0; mx < m; mx++ {
			for y := len(field) * my; (y - len(field)*my) < len(field); y++ {
				expanded[y] += tx[y-(len(field)*my)]
			}
			tx = IncrementWeight(tx)
		}
		temp = IncrementWeight(temp)
	}
	return expanded
}

func main() {
	field := ExpandMap(readMap(os.Args[1]), 4)
	/* for _, l := range field {
		fmt.Println(l, len(l))
	}
